...
- Get aggregated player stats ranked by rating, with a minimum maps played threshold
- Get raw per-match player stats, filterable by map and side
- Get raw player stats filtered by match outcome (win/loss)
...
| `GET /players/stats` | `mapid=0`, `sideid=0`, `limit=20`, `offset=0`, `min_played=10` | Get aggregated player stats ranked by rating |
| `GET /players/stats/raw` | `mapid=0`, `sideid=0`, `limit=20`, `offset=0` | Get raw per-match player stats |
| `GET /players/stats/raw/{outcome}` | `outcome`: `win`\|`lose`, `mapid`, `limit=100`, `offset=0` | Get raw player stats filtered by match outcome |
...
#### Get aggregated player stats ranked by rating
```sh
curl https://api.csapi.de/players/stats
```

#### Get aggregated player stats on a specific map
```sh
curl "https://api.csapi.de/players/stats?mapid=2"
```

#### Get raw player stats filtered by outcome
```sh
curl "https://api.csapi.de/players/stats/raw/win?mapid=2"
```
