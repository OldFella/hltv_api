...
@patch("src.routers.players.format_stats", return_value=[MOCK_PLAYER_STATS])
@patch("src.routers.players.execute_query", return_value=MOCK_PLAYER_STATS)
class TestGetAveragePlayerStats:
    def test_returns_200(self, mock_eq, mock_fs):
        assert client.get("/players/1/stats").status_code == 200

    def test_returns_correct_shape(self, mock_eq, mock_fs):
        data = client.get("/players/1/stats").json()
        assert all(k in data for k in ["id", "name", "k", "d", "adr", "kast", "rating", "maps_played"])

    def test_date_filter(self, mock_eq, mock_fs):
        assert client.get("/players/1/stats?start_date=2024-01-01&end_date=2024-12-31").status_code == 200

    def test_event_filter(self, mock_eq, mock_fs):
        assert client.get("/players/1/stats?event=ESL").status_code == 200


@patch("src.routers.players.execute_query", side_effect=HTTPException(status_code=404, detail="Item not found"))
class TestGetAveragePlayerStatsNotFound:
    def test_not_found(self, mock_eq):
        assert client.get("/players/999999/stats").status_code == 404
